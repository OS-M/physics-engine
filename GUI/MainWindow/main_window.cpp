...
MainWindow::MainWindow(int64_t tickrate, const std::shared_ptr<Engine>& engine)
  ...
  this->resize(engine->GetWorldSize().scaled(QSizeF(1500, 1000), Qt::AspectRatioMode::KeepAspectRatio).toSize());
...
void MainWindow::mousePressEvent(QMouseEvent* event) {
  ...
  std::shared_ptr<PolygonObject> polygon_object;
  if (counter % 2 == 0) {
    polygon_object = std::make_shared<PolygonObject>(
        point, 15, 10);
    polygon_object->SetMass(0.5);
  } else {
    polygon_object = std::make_shared<PolygonObject>(
